...
> **Documentation note:** This README is based on conversations I used with GPT-5.5 throughout the project to question, clarify, and organize what I was learning. Its substance comes from my implementation, experiments, handwritten derivations, and conclusions; the final README text was generated by GPT-5.6 Sol from that project record.
...
The animation makes the neural path look both smoother and shorter in step count. The lower step count is not typical: among the 957 deterministic benchmark cases solved by all three methods, the neural policy used fewer steps than DLS in 26.9% and fewer than the pseudoinverse in 24.8%. The straighter path is a consistent result, however. Its mean end-effector path length was 1.042 times the straight-line distance to the target, compared with 1.199 for DLS and 1.247 for the pseudoinverse. This is an interesting learned behavior, although it does not outweigh the neural policy's lower success rate and longer average rollout.
...
J =
\begin{bmatrix}
\dfrac{\partial x}{\partial \theta_1} &
\dfrac{\partial x}{\partial \theta_2} \\
\dfrac{\partial y}{\partial \theta_1} &
\dfrac{\partial y}{\partial \theta_2}
\end{bmatrix}
$$
...
\boldsymbol{\theta}_{\mathrm{next}} &=
\boldsymbol{\theta} + \alpha\,\Delta\boldsymbol{\theta}
\end{aligned}
$$
...
J^T\!\left(JJ^T + \lambda^2 I\right)^{-1}\mathbf{e}
$$
